...
@dataclass
class FromDictTest(FromDictMixin):
    a: int
    b: int
    c: int
...
def test_from_dict__class_is_not_dataclass():
    ...
    class _FromDictTest(FromDictMixin):
        a: int

    with pytest.raises(TypeError):
        _FromDictTest.from_dict({"a": 1, "b": 2})
...
def test_from_dict__missing_data(data, mapping):
    ...
    with pytest.raises(KeyError):
        FromDictTest.from_dict(data, mapping=mapping)


@pytest.mark.parametrize(
    "data,expected",
    [
        ({"a": 1, "b": 2, "c": 3}, FromDictTest(a=1, b=2, c=3)),
        ({"a": 1, "b": 2, "c": 3, "d": 4}, FromDictTest(a=1, b=2, c=3)),
    ],
    ids=["standard", "additional_keys"],
)
def test_from_dict__return_correct_data_without_mappping(data, expected):
    """Test that properly mapped object is returned without mapping."""
    result = FromDictTest.from_dict(data)
...
def test_from_dict__return_correct_data_with_mappping(data, mapping, expected):
    """Test that properly mapped object is returned with mapping."""
    result = FromDictTest.from_dict(data, mapping=mapping)
    assert result == expected
